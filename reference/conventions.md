...
- **Secrets are redacted in CLI output but keep their column name.** Encrypted columns (e.g. `postgres_url`, `firestore_credentials`, `posthog_api_key`, `stripe_api_key`) appear as `"<redacted>"` rather than the plaintext value. The field name is preserved so agents can still detect "is this set?" without needing the secret.
