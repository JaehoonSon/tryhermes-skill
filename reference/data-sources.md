...
Each uploaded CSV becomes its own **table** inside a single CSV connection. Hermes ingests the file into a managed Postgres instance — for query purposes the source behaves exactly like `postgres` and you write Postgres-flavored SQL.

Adding more data is multi-step in a way the other source types aren't:

```
hermes connections add --csv-file ./users.csv                         # creates the CSV connection + ingests users table
hermes connections add --csv-file ./orders.csv --table-name orders    # appends another table
hermes connections add --csv-file ./events.csv                        # appends another (auto-named)
```

Re-running `add --csv-file` against a CSV connection **accumulates**; the other source types replace. Per-table removal is `hermes connections remove --csv-table <name> --confirm` — see [connections.md](commands/connections.md#hermes-connections-remove-id---confirm---csv-table-name).
...
- **No joins.** You cannot correlate across collections in one query. If you need users *and* their orders, fetch users first, then orders by ID per user. This is expensive — be deliberate.
...
2. **Write a candidate query in the matching dialect.** SQL for SQL family, JSON DSL for Firestore. The example_query field is a literal template — copy and adapt.
...
If the caller sends the wrong dialect (SQL to a Firestore connection, or vice versa), the error response identifies the actual `source_type` and includes the example query for that source — recoverable in one round trip.
...
- **The required projection** is conceptually the same: each result must yield enough to identify and email a person. SQL projects `id, email`; Firestore returns a document whose path supplies the id and whose body supplies the email.
